...
public class CSLCompiler {
        ...
        while ((r = cslCodeReader.read()) != -1) {
            ...
            if (ch == '<') {
                ...
                if (blockType == '$') {
                    ...
                    String body = readTagBody(cslCodeReader);
                    parseOpenExecutionTagBody(body, xslCodeWriter);
                } else if (blockType == '-') {
                    //Code execution control block
                    String body = readTagBody(cslCodeReader);
                    parseEndExecutionTagBody(body, xslCodeWriter);
                } else if (blockType == '{') {
                    //Code execution control block
                    /*contain if:endif; while:endwhile; foreach:endforeach; choose:endchoose; otherwise:endotherwise*/
                    String body = readTagBody(cslCodeReader, "}>");
                ...
                xslCodeWriter.write("<xsl:text disable-output-escaping=\"yes\"><![CDATA[&]]></xsl:text>");
            } else {
                xslCodeWriter.write(ch);
            }
        }
    ...
    protected String readTagBody(Reader reader, String endTag) throws IOException, Exception {
        ...
        return body.substring(0, body.length() - endTag.length());
    }
    ...
    protected void parseOpenExecutionTagBody(String body, Writer writer) throws Exception {
        //----------------------------
        String bodyLow = body.toLowerCase();
        //Special for else
        if (bodyLow.startsWith(ELSE_TAG)) {
            if (bodyLow.length() != ELSE_TAG.length()) {
                throw new Exception("Else tag cannot contain tagbody");
            }
            ExecutionTagStackRecord oldTagRecord = tagsStock.get(tagsStock.size() - 1);
            if (oldTagRecord.executionTag.CodeOpen.equals("if:")) {
                throw new Exception("You can use else only in if tag");
        ...
        for (ExecutionTag tag : exectutionTags) {
            if (bodyLow.startsWith(tag.openString)) {
                /*if ((!tag.openString.substring(tag.openString.length() - 1).equals(":"))
                && tag.openString.length() != body.length()) {
                throw new Exception("The tag cannot contain tagbody");
                }*/
                tagsStock.add(new ExecutionTagStackRecord(tag, body));
                writer.write(tag.CodeOpen.replaceAll("%", body.substring(tag.openString.length())));
                return;
    ...
    protected void parseEndExecutionTagBody(String body, Writer writer) throws Exception {
        //----------------------------
        String bodyLow = body.toLowerCase();
        if(tagsStock.size()==0){
            throw new Exception("Tag stack is empty. You dont open any tag and you try close it with tagbody"+body+" near "+lineNumber);
        }
        int last = tagsStock.size()-1;
        ExecutionTagStackRecord lastTag=tagsStock.get(last);
        if(bodyLow.startsWith(lastTag.executionTag.endString)){
            writer.write(lastTag.executionTag.CodeEnd);
            tagsStock.remove(last);
            return;
        }
        throw new Exception("Don't know tag with body: '" + body + "' near line " + lineNumber);
    }
    ...
    private final static void printText(String filename, Writer out) throws IOException {
        InputStream is = CSLCompiler.class.getResourceAsStream(filename);
